...
from model.regressor.example_model import load_pickle, create_atoms, create_ijbonddict, extract_fingerprints, create_adjacency, split_sequence, luciferase_contact_map
from model.regressor.utils import PredefinedNoiseScheduleDiscrete, DiscreteUniformTransition, BlosumTransition
...
class EGNN_NET(torch.nn.Module):
    def __init__(self, input_feat_dim, hidden_channels, edge_attr_dim, dropout=0.0, n_layers=1, output_dim=20, embedding=False, embedding_dim=64, mlp_num=2, update_edge=True, embed_ss=-1, norm_feat=False):
        ...
        if embedding:
            ...
            self.time_mlp = nn.Sequential(nn.Linear(1, hidden_channels), nn.SiLU(), nn.Linear(hidden_channels, input_feat_dim))
        
            self.ss_mlp = nn.Sequential(nn.Linear(8, hidden_channels), nn.SiLU(), nn.Linear(hidden_channels, input_feat_dim))
            

        for i in range(n_layers):
            if embedding:
                layer = EGNN_Sparse(embedding_dim, m_dim=hidden_channels, edge_attr_dim=embedding_dim, dropout=dropout, mlp_num=mlp_num, update_edge=self.update_edge, norm_feats=norm_feat)
            else:
                layer = EGNN_Sparse(input_feat_dim, m_dim=hidden_channels, edge_attr_dim=edge_attr_dim, dropout=dropout, mlp_num=mlp_num, update_edge=self.update_edge, norm_feats=norm_feat)
    ...
    def forward(self, data,time): 
        ...
        extra_x = extra_x.cuda()
        x = x.cuda()
        x = torch.cat([x,extra_x],dim=1).cuda()
        if self.embedding:
        ...
        for i, layer in enumerate(self.mpnn_layes):
            ...
            x = torch.cat([corr, feats], dim=-1).cuda()
...
class KcatDiffuser(nn.Module):
    ...
    def compute_batched_over0_posterior_distribution(self,X_t,Q_t,Qsb,Qtb,data):
        ...
        Qt_T = Q_t.transpose(-1,-2).cuda()
        X_t_ = X_t.unsqueeze(dim=-2).cuda()
    ...
    def cond_fn(self, t, data, regressor,smiles,sequence,structure,input_properties,target=100):
        ...
        with torch.enable_grad():
            ...
            atom_dict = load_pickle('./diffusion/results/dict/atom_dict.pickle')
            bond_dict = load_pickle('./diffusion/results/dict/bond_dict.pickle')
            edge_dict = load_pickle('./diffusion/results/dict/edge_dict.pickle')
            ...
            print(f"Value of x_in: {x_in}")
            pred, ori = regressor.forward(inputs, layer_output, dropout, batch, x_in, edge_index, t)
            pred = pred.to(device)
            #pred = torch.tensor(pred.item(), dtype=torch.float).to(device)
            ori_in = ori.float().detach().requires_grad_(True)
            #print(f"Value of pred: {pred}")
            #print(f"Value of ori_in: {ori_in}")
            
            target = torch.tensor(target, dtype=torch.float).to(device)
            tt = target.repeat(1, pred.size(1))
            #print(f"length of tt: {tt.shape}")

            mse = criterion(pred, tt)
            #print(f"Value of mse: {mse}")

            t_int = int(t[0].item() * 500)

            # calculate gradient of mse with respect to x
            grad_ori = torch.autograd.grad(mse, ori, retain_graph=True)[0]
            #print(f"Value of grad_ori: {grad_ori}")

            return grad_ori
    
    def sample_p_zs_given_zt_regressor(self,t,s,zt,data,cond,diverse,step,last_step,regressor,smiles,sequence,structure,input_properties):
        """
        sample zs~p(zs|zt)
        """
        alpha_s_bar = self.noise_schedule.get_alpha_bar(t_normalized=s)
        ...
        t = t.to(device)
        pred = self.model(noise_data,t*self.timesteps)
        pred_X = F.softmax(pred,dim = -1).cuda() 
        ...
        p_s_and_t_given_0_X = self.compute_batched_over0_posterior_distribution(X_t=zt,Q_t=Qt,Qsb=Qsb,Qtb=Qtb,data=data)#[N,d0,d_t-1] 20,20 approximate Q_t-s with Qt 
        p_s_and_t_given_0_X = p_s_and_t_given_0_X.cuda()
        weighted_X = pred_X.unsqueeze(-1) * p_s_and_t_given_0_X #[N,d0,d_t-1]
        unnormalized_prob_X = weighted_X.sum(dim=1)             #[N,d_t-1]
        unnormalized_prob_X[torch.sum(unnormalized_prob_X, dim=-1) == 0] = 1e-5
        prob_X = unnormalized_prob_X / torch.sum(unnormalized_prob_X, dim=-1, keepdim=True)  #[N,d_t-1]
        ...
        # guidance
        lamb = 13
        grad_x = self.cond_fn(t, noise_data, regressor,smiles,sequence,structure,input_properties)
        print(f"Value of lamb: {lamb}")
    ...
    def ddim_sample_regressor(self,data,smiles,sequence,structure,cond = False,diverse=False,stop = 0,step=50,regressor=None,input_properties=None):
        limit_dist = torch.ones(20)/20
        num_node = data.x.shape[0]
        zt = self.sample_discrete_feature_noise(limit_dist = limit_dist,num_node = num_node) #[N,20] one hot 
        zt = zt.to(data.x.device)
        optimizer = Adam([zt], lr=1e-9)
        ...
        for s_int in tqdm(list(reversed(range(stop, self.timesteps,step)))): #500
            ...
            t_norm = t_array /self.timesteps
            if regressor:
                zt, final_predicted_X  = self.sample_p_zs_given_zt_regressor(t_norm, s_norm,zt, data,cond,diverse,step,last_step=s_int==stop,regressor=regressor,smiles=smiles,sequence=sequence,structure=structure,input_properties=input_properties)
            else:
                zt, final_predicted_X  = self.sample_p_zs_given_zt(t_norm, s_norm,zt, data,cond,diverse,step,last_step=s_int==stop)
...
if __name__ == "__main__" :
    ...
    parser.add_argument('--Date', type = str,default='Jul_2th',
                        help='Date of experiment')
    
    parser.add_argument('--train_dir', type = str,default='dataset/cath/train/',
                        help='path of training data')
    
    parser.add_argument('--val_dir', type = str,default='dataset/cath/validation/',
                        help='path of val data')
    
    parser.add_argument('--test_dir', type = str,default='dataset/cath/test/',
                        help='path of test data')

    parser.add_argument('--bc_train_dir', type = str,default='dataset/process/train/',
                        help='path of training data')
    
    parser.add_argument('--bc_val_dir', type = str,default='dataset/process/validation/',
                        help='path of val data')
    
    parser.add_argument('--bc_test_dir', type = str,default='dataset/process/test/',
                        help='path of test data')      
    ...
    parser.add_argument('--noise_type', type = str,default='blosum',help='the type of noise, uniform or blosum')
    ...
    elif config['dataset'] == 'BRENDA':
        print('train on CATH & BRENDA dataset')
        train_ID,val_ID,test_ID = os.listdir(config['bc_train_dir']),os.listdir(config['bc_test_dir']),os.listdir(config['bc_test_dir'])
        train_dataset = Cath(train_ID,config['bc_train_dir'])
        val_dataset = Cath(val_ID,config['bc_test_dir'])
        test_dataset = Cath(val_ID,config['bc_test_dir'])
    ...
    ckpt = torch.load('diffusion/results/weight/BLOSUM_3M_small.pt', map_location=device)#UNIFORM_3M_small.pt
    config = ckpt['config']
    ...
    diffusion_model = KcatDiffuser(model,timesteps=config['timesteps'],objective=config['objective'],config=config)
    
    trainer = Trainer(config,diffusion_model,train_dataset,val_dataset,test_dataset,train_batch_size = config['batch_size'],train_lr=config['lr'],weight_decay = config['wd'],ema_decay= config['ema_decay'])
    trainer.train()
